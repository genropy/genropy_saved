...
var t=this.chart.theme,_9,_a,_b,f,_d=this.opt.gap<this._vScaler.scale/3?this.opt.gap:0,_e=(this._vScaler.scale-2*_d)/this.series.length;
...
var _11=Math.max(0,this._hScaler.bounds.lower),_12=_5.l+this._hScaler.scale*(_11-this._hScaler.bounds.lower),_13=_4.height-_5.b-this._vScaler.scale*(1.5-this._vScaler.bounds.lower)+_d+_e*(this.series.length-i-1);
for(var j=0;j<run.data.length;++j){
var v=run.data[j],_16=this._hScaler.scale*(v-_11),_17=_e,w=Math.abs(_16);
if(w>=1&&_17>=1){
var _19=s.createRect({x:_12+(_16<0?_16:0),y:_13-this._vScaler.scale*j,width:w,height:_17}).setFill(_b).setStroke(_a);
run.dyn.fill=_19.getFill();
run.dyn.stroke=_19.getStroke();
}
}
run.dirty=false;
}
