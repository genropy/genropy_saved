...
this.cleanGroup();
var s=this.group,ta=this.chart.theme.axis,_b=this._vAxis.getScaler();
if(this.opt.hMinorLines&&_b.minor.tick){
for(var i=0;i<_b.minor.count;++i){
var y=_7.height-_8.b-_b.scale*(_b.minor.start-_b.bounds.lower+i*_b.minor.tick);
s.createLine({x1:_8.l,y1:y,x2:_7.width-_8.r,y2:y}).setStroke(ta.minorTick);
}
}
if(this.opt.hMajorLines&&_b.major.tick){
for(var i=0;i<_b.major.count;++i){
var y=_7.height-_8.b-_b.scale*(_b.major.start-_b.bounds.lower+i*_b.major.tick);
s.createLine({x1:_8.l,y1:y,x2:_7.width-_8.r,y2:y}).setStroke(ta.majorTick);
}
}
_b=this._hAxis.getScaler();
if(this.opt.vMinorLines&&_b.minor.tick){
for(var i=0;i<_b.minor.count;++i){
var x=_8.l+_b.scale*(_b.minor.start-_b.bounds.lower+i*_b.minor.tick);
s.createLine({x1:x,y1:_8.t,x2:x,y2:_7.height-_8.b}).setStroke(ta.minorTick);
}
}
if(this.opt.vMajorLines&&_b.major.tick){
for(var i=0;i<_b.major.count;++i){
var x=_8.l+_b.scale*(_b.major.start-_b.bounds.lower+i*_b.major.tick);
