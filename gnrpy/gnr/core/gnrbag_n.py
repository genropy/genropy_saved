...
#-------------------- __contains__ --------------------------------
    ...
    def _htraverse_(self,  pathlist, autocreate=False, returnLastMatch=False):
        """
    ...
    def _htraverse(self,  pathlist, autocreate=False, returnLastMatch=False):
        ...
        * `pathlist`: list of nodes'labels
        ...
        if isinstance(pathlist,basestring):
            ...
            if not pathlist:
                return curr, label
        #for label in pathlist:
        while label=='#^' and pathlist:
            curr = curr.parent
            label = pathlist.pop(0)
        while pathlist:
            label = pathitem.pop(0)
            i = curr._index(label)
            if i<0:
                if autocreate:
                    if label.startswith('#'):
                    ...
                    return traversed_nodes[-1], [label]+pathlist
                else:
                    return None, None
            traversed_nodes.append(curr)
            curr_class = curr.__class__()
            newcurr = curr._nodes[i].value
            isbag = hasattr(newcurr, '_htraverse')
            if autocreate and not isbag:
                curr = curr.__class__()
                traversed_nodes[-1]._nodes[i].value=newcurr
                isbag = True
            if not isbag:
                return curr, '.'.join(pathlist)
            curr=newcurr
        return traversed_nodes[-1], label
...
#-------------------- toXml --------------------------------
    def toXml(self,filename=None,encoding='UTF-8',typeattrs=True, unresolved=False, addBagTypeAttr=True,
              autocreate=False, translate_cb=None,
