...
class WK2pdf(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(args)<2:
            self.app.quit(1)
            raise Exception('You need to specify atleast one input file, and exactly one output file\nUse - for stdin or stdout\n\n')
        args = list(args)
        self.out = args.pop(-1)
        self.in_ = args
        self.pageNum = 0
    ...
    def printPage(self):
        ...
        printer.setPageMargins(self.margin_left[0], self.margin_top[0], self.margin_right[0], self.margin_bottom[0], self.margin_left[1])
...
if __name__=='__main__':
    ...
    parser.add_option('-n','--disable-javascript', dest="disable_javascript", default = False, action="store_true", help="Do not allow webpages to run javascript")
    parser.add_option('-d','--dpi', dest="dpi", default=-1, help="Change the dpi explicitly")
    parser.add_option('-g','--grayscale', dest="colorMode", default = True,  action="store_false", help="PDF will be generated in grayscale")
    parser.add_option('-l','--lowquality', dest="resolution", default = False, action="store_true", help="Generates lower quality pdf/ps. Useful to shrink the result document space")
    parser.add_option('-B','--margin-bottom', dest="margin_bottom", default='10mm', help="Set the page bottom margin (default 10mm)")
    parser.add_option('-L','--margin-left', dest="margin_left", default='10mm', help="Set the page left margin (default 10mm)")
    parser.add_option('-R','--margin-right', dest="margin_right", default='10mm', help="Set the page right margin (default 10mm)")
    parser.add_option('-T','--margin-top', dest="margin_top", default='10mm', help="Set the page top margin (default 10mm)")
    parser.add_option('--no-background', dest="background", default=True, action="store_false", help="Do not print background")
    parser.add_option('--user-style-sheet', dest="user_style_sheet", default='', help="Specify a user style sheet, to load with every page")
    parser.add_option('-O','--orientation', dest="orientation", default='V', help="Set orientation to Landscape or Portrait")
    ...
    parser.add_option('--redirect-delay', dest="jsredirectwait",  default=200, help="Wait some miliseconds for js-redirects")
    parser.add_option('-V','--version', dest="version", action="store_false", help="Output version information an exit")
    parser.add_option('--enable-plugins', dest="enable_plugins", default=False, action="store_true", help="Enable installed plugins (such as flash)")
    parser.add_option('--use-xserver', dest="use_x11", default = False, action="store_true", help="Use the X server (some plugins and other stuff might not work without X11)")
