...
class GnrBaseWebPage(GnrObject):
    ...
    def saveRecordCluster(self, data, table=None, _nocommit=False, rowcaption=None, _autoreload=False,
        ...
        if not self.checkTablePermission(table,'readonly,hidden'):
            raise self.exception('business_logic',message='Table %s is readonly for user %s' %(table,self.user))
        if newrecord and not self.checkTablePermission(table,'add'):
            raise self.exception('business_logic',message='User %s cannot add record in table %s' %(self.user,table))
        elif not self.checkTablePermission(table,'upd'):
            raise self.exception('business_logic',message='User %s cannot update record in table %s' %(self.user,table))
        if newrecord:
    ...
    def deleteRecordCluster(self, data, table=None, **kwargs):
        ...
        if not self.checkTablePermission(table,'del'):
            raise self.exception('business_logic',message='User %s cannot delete records in table %s' %(self.user,table))
        try: #try:
