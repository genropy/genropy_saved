   ...
   #        return result[('%s_%s' % (target_fld, from_fld)).replace('.', '_')]
    ...
    def dbSelect(self, dbtable=None, columns=None, auxColumns=None, hiddenColumns=None, rowcaption=None,
        ...
        if _id:
            ...
            if invalidItem:
                resultcolumns.append("""(%s IS TRUE) AS _is_invalid_item""" %invalidItem)
                #resultcolumns.append("""(CASE WHEN %s IS TRUE THEN 'is_invalid_row' ELSE '' END) AS _customclasses_invaliditem""" %invalidItem)
