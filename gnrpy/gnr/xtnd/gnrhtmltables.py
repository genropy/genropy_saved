...
class TableBuilder(object):
    ...
    def fromServerCursor(self, n):
        ...
        thead = self.thead or '<tr>%s</tr>' % '\n'.join(
                ['<th>%s</th>' % self.source.tblstruct.columns.get(field, field).name_short for field in fields])
        self.row_template = self.row_template or '<tr value="$pkey" class="$odd_even">%s</tr>' % '\n'.join(
                ['<td>$%s</td>' % field for field in fields])
        yield '<thead>\n%s\n</thead><tbody>' % thead
...
class StringTableBuilder(TableBuilder):
    def doTable(self):
        html = []
        openpage = gnrstring.templateReplace('<table class="full_page $tableclass" id="maintable">',
                                             dict(tableclass=self.tableclass))
...
def tablePage(page, source=None, title='', tableclass='', thead='', row_template='', row_cb=None, skin='gnr_blue',
    ...
    builder = factory(page, source=source, title=title, tableclass=tableclass, thead=thead, row_template=row_template,
                      ...
                      header=header, footer=footer, filename=filename, tot_cb=tot_cb,
                      row_template_totals=row_template_totals,
