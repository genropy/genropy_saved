        ...
        for sname, sbag in self.folder4dstructBag.items():
            ...
            if tablesbag:
                for tbag in tablesbag.values():
                    ...
                    namesdict[tableSql] = None # in namesdict put the table with area name avoiding multi area duplicates
    ...
    def buildField(self, table, fldbag):
        ...
        if fldbag['relate']:
            ...
            if sqltarget:
                ...
                sqltarget = self.nameConverter(table=fldbag['relate.tablegnr'], field=fldbag['relate.fieldgnr'], mode='GNR')
                case_insensitive = True
            if sqltarget:
