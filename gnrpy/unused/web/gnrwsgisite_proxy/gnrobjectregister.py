...
class SiteRegister(object):
    ...
    def new_connection(self, connection_id, connection):
        assert not self.c_register.exists(
                connection_id), 'SITEREGISTER ERROR: connection_id %s already registered' % connection_id
    ...
    def cleanup(self):
        ...
        if lastSiteCleanup and (time.time()-lastSiteCleanup < self.site.cleanup_interval):
            return
...
class BaseRegister(object):
    ...
    def set_index(self, register_item, index_name=None):
        ...
        self.log('set_index', register_item_id=register_item['register_item_id'], index_name=index_name,
                 ind_key=ind_key)
    ...
    def lock_register_item(self, register_item_id, max_retry=None,
                           lock_time=None,
                           retry_time=None):
        return self.sd.lock(self.item_key(register_item_id), max_retry=max_retry, lock_time=lock_time,
