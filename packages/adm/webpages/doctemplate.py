...
class GnrCustomWebPage(object):
    ...
    def formBase(self, parentBC, disabled=False, **kwargs):
        ...
        self.metadataForm(tc.contentPane(title='!!Metadata', pageName='metadata', datapath='.metadata'),
                          disabled=disabled)
    ...
    def previewPane(self, bc):
        ...
        box.button('!!Previous',
                   action='idx = idx>0?idx-1:10; SET preview.selected_id = pkeys[idx]; SET preview.idx = idx;',
                   idx='=preview.idx', pkeys='=preview.pkeys',
                   iconClass="tb_button icnNavPrev", showLabel=False)
        box.button('!!Next',
                   action='idx = idx<=pkeys.length?idx+1:0; SET preview.selected_id = pkeys[idx]; SET preview.idx = idx;'
                   , idx='=preview.idx', pkeys='=preview.pkeys',
        ...
        bc.contentPane(region='center', padding='10px').div(height='100%', background='white').div(
                innerHTML='^preview.renderedtemplate')
