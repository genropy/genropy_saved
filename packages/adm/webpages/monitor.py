...
class GnrCustomWebPage(object):
    ...
    def main(self, root, **kwargs):
        ...
        for pkg in self.db.packages.values():
            ...
            if auth:
                cblist = sorted([func_name for func_name in dir(self) if func_name.startswith('monitor_%s_' %pkg.name)])
                if len(cblist)>0:
    ...
    def rpc_monitorUpdate(self):
        result = Bag()
        for pkg in self.db.packages.values():
            cblist = sorted([func_name for func_name in dir(self) if func_name.startswith('rpc_monitor_%s_' %pkg.name)])
            if len(cblist)>0:
                result[pkg.name] = Bag()
                for cbname in cblist:
