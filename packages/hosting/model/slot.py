...
class Table(object):
    ...
    def set_slots(self, config, instance_id):
        ...
        freeSlots = dict([(slot_type,[r['id'] for r in currentConfig[slot_type] if not r['used']]) for slot_type in currentConfig])
    ...
    def use_slot(self, slotname, slot_type, instance_id):
        free_id=self.query('$id',where='$instance_id =: instance_id AND $type =:slot_type AND $used<>TRUE',slot_type=slot_type,
