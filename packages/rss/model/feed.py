...
class Table(object):
    def config_db(self, pkg):
        ...
        tbl.column('title',name_long='!!Title',validate_notnull=True,validate_notnull_error='!!Required',validate_case='c'
        )    
    ...
    def getFeeds(self,user=None):
        b = self.query(where='$username=:user',user=user).selection().totalize(group_by=['topic','title'],keep=['id','url'],key='id')     
        def epuratore(node):
            if node.value:
                node.attr=None
        b.walk(epuratore)
