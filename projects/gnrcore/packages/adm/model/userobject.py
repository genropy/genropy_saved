...
class Table(object):
    def config_db(self, pkg):
        ...
        tbl.column('code', name_long='!!Code', unique='y') # a code unique for the same type / pkg / tbl
        tbl.column('objtype',size=':255', name_long='!!Object Type', indexed='y')
        tbl.column('pkg', name_long='!!Package') # package code
        tbl.column('tbl', name_long='!!Table') # full table name: package.table
        tbl.column('userid',size=':255', name_long='!!User ID', indexed='y')
        tbl.column('description', 'T', name_long='!!Description')
