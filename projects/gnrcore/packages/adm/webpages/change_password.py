...
class GnrCustomWebPage(object):
    ...
    def formPage(self, sc, bottom, changeusername):
        ...
        #-------------------widget definition--------------------
        fbp = client.formbuilder(cols=1, datapath='passchange', border_spacing='5px', margin='1em',
    ...
    def rpc_doChangePassword(self, data, **kwargs):
        ...
        if data['username']:
            self.db.table('adm.user').update(
                        {'id': self.avatar.userid, 'md5pwd': newpass, 'username': data['username']})
        else:
