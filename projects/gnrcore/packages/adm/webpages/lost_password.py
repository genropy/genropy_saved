...
class GnrCustomWebPage(object):
    ...
    def main(self, root, userid=None, **kwargs):
        sc, top, bottom = self.pbl_rootStackContainer(root, '!!Lost Password', height='120px', width='35em',
                                                      centered=True)
        if userid:
    ...
    def pageForm(self, sc, bottom):
        ...
        fb = center.formbuilder(cols=1, datapath='newpass', border_spacing='5px', margin='auto', margin_top='2em',
                                onEnter='FIRE confirmbtn=true')
    ...
    def _doNewPassword(self, userid, **kwargs):
        ...
        recordBag['link'] = self.externalUrl('adm/change_password',
                                             _user_login='%s:%s' % (recordBag['username'], recordBag['newpassword']))
