...
class GalleryForm(Form):
    ...
    def th_form(self,form):
        bc = form.center.borderContainer()
        top = bc.contentPane(region='top',height='20%',overflow='auto').div('^.record.gallery_doc')
       ...
       #        bc=bc.js_widget,localIframeUrl='^.localIframeUrl',_onBuilt=True)
        ...
        bc.dataFormula('.localIframeUrl','sourcebag.len()==1?sourcebag.getItem("_base_.url"):null',
        ...
        bc.contentPane(region='center',overflow='hidden').iframe(src='^.localIframeUrl',
                                                src__source_viewer='stack',
                                                src_cm_theme='night',
                                                src_cm_font_size='1.2em',
    ...
    def th_options(self):
        return dict(hierarchical=False,showtoolbar=False)

    @public_method
    def th_onLoading(self, record, newrecord, loadingParameters, recInfo):
        doctable = self.db.table('docu.documentation')
        doctable.checkSourceBagModules(record)
        ...
        if rsttable:
            rst = '%s\n\n%s' %(rst,rsttable) 
        record['gallery_doc'] = self.site.getService('rst2html')(rst)
