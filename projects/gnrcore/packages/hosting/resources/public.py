...
class TableHandlerMain(BaseComponent):
    def onMain_hosting_addOn(self):
        r = filter(lambda r: r[0]==self.maintable,self.application.config.getItem('packages.gnrcore:hosting.synctables').digest('#a.tbl,#a.permission'))
        if not r:
            return
        tbl,permission = r[0]
    ...
    def __viewCustomization(self,view): #poi ci passo il th direttamente
        bar = view.top.bar.replaceSlots('addrow','addrow,hosting_propagate')        
