...
},datumChange:function(_b1,_b2,_b3){
if(this._canWrite){
var row=this.getRow(_b2);
var _b5=this.fields._nameMaps[_b3+".idx"];
this.store.setValue(row.__dojo_data_item,_b5,_b1);
}else{
this.notify("DatumChange",arguments);
}
},insertion:function(){
...
var _b8=this.data[_b6[i]].__dojo_data_item;
this._currentlyProcessing.push(_b8);
this.store.deleteItem(_b8);
}
this._removeItems(_b6);
this._currentlyProcessing=[];
},_removeItems:function(_b9){
...
var col=Math.abs(_bb)-1;
this.sortFields=[{"attribute":this.fields.values[col].name,"descending":(_bb>0)}];
this.refresh();
},refresh:function(){
this.clearData(true);
this.requestRows();
},clearData:function(_bd){
this._rowIdentities={};
this.pages=[];
this.bop=this.eop=-1;
this.count=0;
this.setData((_bd?this.store:[]));
},processError:function(_be,_bf){
console.log(_be);
}});
}
