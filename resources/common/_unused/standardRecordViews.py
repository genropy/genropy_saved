...
class RecordAndViews(BaseComponent):
    ...
    def formToolbar(self, where):
        ...
        if self.userCanDelete() or self.userCanWrite():
            trbtn.button('!!Unlock', float='right',fire='status.unlock', iconClass="tb_button db_lock", showLabel=False,hidden='^status.unlocked')
            trbtn.button('!!Lock', float='right',fire='status.lock', iconClass="tb_button db_unlock", showLabel=False,hidden='^status.locked')
    ...
    def formController(self,pane):
        ...
        pane.dataRpc('form.save_result','save', data='=form.record', _POST=True, isValid='^form.isValid' , _if='isValid')
