...
class DynamicForm(BaseComponent):
    ...
    def df_dynamicFieldsTestPane(self,pane,df_table=None,df_pkey=None,**kwargs):
        pane.contentPane(_workspace=True).remote(self.df_remoteDynamicForm,df_table=df_table,df_pkey=df_pkey,df_is_new=True,cachedRemote=True,**kwargs)
    ...
    def df_appendDynamicFields(self,pane,field=None,**kwargs):
        ...
        pane.remote(self.df_remoteDynamicGroup,df_table=df_table.fullname,
                            _fired='^#FORM.changed_df_type_%s' %field,
                            df_pkey='=#FORM.record.%s' %df_field,datapath='#FORM.record.%s' %field,
                            df_is_new='==!this.getRelativeData("#FORM.record.%s")' %field,fbattrs=fbattrs,
                            _onRemote="""
    ...
    def _df_handleFieldValidation(self,attr,fields):
        ...
        if attr.get('field_visible'):
            ...
            tpv = (','.join([str(x['code']) for x in fields if x['code'] in condition]),"", "",attr['code'])
