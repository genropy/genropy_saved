...
class FiltersHandler(BaseComponent):
    ...
    @struct_method
    def fh_slotbar_filtermenu(self, pane,**kwargs):
...
class TagsHandler(BaseComponent):
    ...
    @struct_method
    def th_slotbar_tagsbtn(self,pane,mode=None,**kwargs):
        pane.slotButton('!!Tags', action='genro.publish("tagshandler_open",{call_mode:mode})',mode=mode,iconClass="icnTag", showLabel=False,**kwargs)
    ...
    def tags_main(self, pane):
        pane.dataController("""FIRE #linktag_dlg.open={call_mode:mode};""",subscribe_tagshandler_open=True)
